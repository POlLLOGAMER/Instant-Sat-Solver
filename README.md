...
```
Observations: If the number of variables is equal to the number of clauses, even if there is only 1 variable, it will be solved in 1 attempt, if the number of clauses is greater, it does not matter how greater or less than the number of variables (Only applies if the number of variables is greater than 14), it will always be done in 1 attempt, but if the variables are less than the clauses, and the variables are less than 15, it will take time. exponentially infinite
